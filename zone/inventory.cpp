...
bool Client::SwapItem(MoveItem_Struct* move_in) {
	...
	//imperium update 6-11-2022: changed ranges and added specific check for both shared bank and shared bank bags
	...
	//imperium update 6-11-2022: changes ranges and added specific check for both shared bank and shared bank bags
	...
	if (((with && with->IsClient() && dst_slot_id >= EQ::invslot::TRADE_BEGIN && dst_slot_id <= EQ::invslot::TRADE_END) ||
		(dst_slot_id >= EQ::invslot::SHARED_BANK_BEGIN && src_slot_id <= EQ::invslot::SHARED_BANK_END || src_slot_id >= EQ::invbag::SHARED_BANK_BAGS_BEGIN && src_slot_id <= EQ::invbag::SHARED_BANK_BAGS_END))
	...
	&& RuleI(World, FVNoDropFlag) == 0 || RuleI(Character, MinStatusForNoDropExemptions) < Admin() && RuleI(World, FVNoDropFlag) == 2) {
		...
		LogError("WorldKick() of Player [{}](id:[{}], acct:[{}]) due to 'NoDrop Hack' detection >> SlotID:[{}], ItemData:[{}], Destination:[{}]",
			GetName(), CharacterID(), AccountID(), src_slot_id, ndh_item_data.c_str(), dst_slot_id);
		ndh_inst = nullptr;
