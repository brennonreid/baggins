...
bool SharedDatabase::SaveInventory(uint32 char_id, const EQ::ItemInstance* inst, int16 slot_id) {
	...
	if (slot_id >= EQ::invslot::SHARED_BANK_BEGIN && slot_id <= EQ::invslot::SHARED_BANK_END || slot_id >= EQ::invbag::SHARED_BANK_BAGS_BEGIN && slot_id <= EQ::invbag::SHARED_BANK_BAGS_END) {
...
#undef F
	...
	for (auto row = results.begin(); row != results.end(); ++row) {
		...
		//item.BagSlots = (uint8)std::min(atoi(row[ItemField::bagslots]), 50); // FIXME: remove when big bags supported
		item.BagSlots = (uint8)atoi(row[ItemField::bagslots]); // FIXED: working on big bag support!
